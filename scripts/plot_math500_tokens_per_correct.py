...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Plot MATH-500 total-token consumption divided by the number "
    ...
    parser.add_argument(
        ...
        default=(
            ...
            / "results"
            / "summary"
            / "math500_main_v3_qwen25_7b__summary.csv"
        ),
    )
    parser.add_argument(
        "--output-prefix",
        ...
        default=(
            ...
            / "results"
            / "figures"
            / "math500_main_v3_qwen25_7b__tokens_per_correct_bar"
        ),
    )
    return parser.parse_args()


def load_math500_summary(path: Path) -> pd.DataFrame:
    ...
    if set(protocols) != set(PROTOCOL_ORDER) or len(protocols) != 6:
        raise ValueError(
            "Expected exactly the six MATH-500 protocols; found: "
            f"{protocols}"
        )
    ...
    if not ordered["n"].eq(500).all():
        counts = dict(zip(ordered["protocol"], ordered["n"]))
        raise ValueError(
            "Expected 500 MATH-500 items for every protocol; found: "
            f"{counts}"
        )
...
def create_figure(summary: pd.DataFrame) -> plt.Figure:
    ...
    ax.set_title("MATH-500: Amortized token consumption per correct answer")
...
def main() -> None:
    args = parse_args()
    summary = load_math500_summary(args.summary.resolve())
